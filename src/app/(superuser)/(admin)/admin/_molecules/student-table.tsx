...
import { approveStudent, rejectStudent, bulkApprove, bulkReject } from "@/actions";
import { toast } from "react-toastify";
import { useQueryClient } from "@tanstack/react-query";
import { CheckSquare, Square, CheckCheck, X } from "lucide-react";
import { Button } from "@/components/ui/button";

export default function StudentTable({ data, isLoading }: { isLoading: boolean; data: any }) {
  ...
  const filteredStudents = (data ?? []).filter((s: any) => {
    ...
    return name.includes(q) || email.includes(q) || (s.courseOfStudy ?? "").toLowerCase().includes(q);
  });
  ...
  return (
    <div className="border rounded-xl overflow-x-auto bg-white shadow">
      <div className="p-4 flex justify-between items-center gap-3 flex-wrap">
        ...
        />
        <Link href="student/new" className={buttonVariants({ variant: "default" })}>Add Student</Link>
      </div>
      ...
      {selected.size > 0 && (
        <div className="flex items-center gap-3 mx-4 mb-3 bg-primary/5 border border-primary/20 rounded-xl px-4 py-2.5">
          ...
          <Button size="sm" disabled={isActing} onClick={() => bulkApp({ type: "student", ids: Array.from(selected) })} className="gap-1.5 bg-emerald-600 hover:bg-emerald-700">
            <CheckCheck className="w-3.5 h-3.5" /> Approve All
          </Button>
          <Button size="sm" variant="outline" disabled={isActing} onClick={() => bulkRej({ type: "student", ids: Array.from(selected) })} className="gap-1.5 border-red-200 text-red-600 hover:bg-red-50">
      ...
      <table className="min-w-full text-sm text-left">
        ...
        <tbody>
          {filteredStudents.map((s: any, index: number) => (
            <tr key={s.id} className="border-b hover:bg-gray-100 transition cursor-pointer" onClick={() => router.push(`student/${s.id}`)}>
              <td className="p-3" onClick={(e) => e.stopPropagation()}>
                <button onClick={() => toggleOne(s.id)}>
                  {selected.has(s.id) ? <CheckSquare className="w-4 h-4 text-primary" /> : <Square className="w-4 h-4 text-gray-300" />}
                </button>
              </td>
              <td className="p-3">
                <div className="rounded-full h-10 w-10">
                  <Image src={s?.profileImage || "/applicant.png"} width={40} height={40} className="rounded-full w-full h-full object-cover" alt="profile" />
                </div>
              </td>
              <td className="p-3">{s.firstName} {s.lastName}</td>
              <td className="p-3">{s?.user?.email}</td>
              <td className="p-3">{s?.courseOfStudy}</td>
              <td className="p-3">{s?.user?.isActive ? "Active" : "Inactive"}</td>
              <td className="p-3">
                <span className={`px-2 py-1 rounded-full text-xs font-semibold ${studentStatusStyle[s.status as keyof typeof studentStatusStyle]}`}>
                  {s.status}
                </span>
              </td>
              <td className="p-3" onClick={(e) => e.stopPropagation()}>
                <div className="flex gap-2">
                  <button disabled={isActing} onClick={() => approve({ studentId: s.id })} className="px-3 py-1 cursor-pointer text-xs rounded bg-green-100 text-green-700 hover:bg-green-200 disabled:opacity-50">Approve</button>
                  <button disabled={isActing} onClick={() => reject({ studentId: s.id })} className="px-3 py-1 cursor-pointer text-xs rounded bg-red-100 text-red-700 hover:bg-red-200 disabled:opacity-50">Remove</button>
                </div>
              </td>
            </tr>
          ))}
