...
async function fetchPreview(): Promise<PreviewOpportunity[]> {
  try {
...
export async function OpportunitiesPreview() {
  const opportunities = await fetchPreview();
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-12">
          <div>
            ...
            <h2 className="mt-3 text-3xl sm:text-4xl font-bold text-gray-950 leading-tight tracking-tight">
              Placements open right now
            </h2>
            <p className="mt-2 text-gray-500 text-sm sm:text-base max-w-md">
              A snapshot of what&apos;s available. Hundreds more waiting on the
              opportunities page.
            </p>
          </div>
          <Link
              ...
              <div className="p-5 flex flex-col gap-3 flex-1">
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <h3 className="font-semibold text-sm text-gray-900 capitalize truncate group-hover:text-primary transition-colors">
                      {opp.title}
                    </h3>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {[
                    ...
                    { icon: <Clock className="w-3 h-3" />, text: `${opp.duration} mo` },
                    {
                      ...
                      text: opp.stipend === "paid" ? "Paid" : "Unpaid",
                    },
                  ].map((tag) => (
                    <span
                      key={tag.text}
                      className="inline-flex items-center gap-1 text-[11px] font-medium bg-gray-50 border border-gray-100 px-2.5 py-0.5 rounded-lg text-gray-600 capitalize"
                    >
                      {tag.icon}
                      {tag.text}
                    </span>
                ...
                <div className="flex items-center gap-1 text-[11px] text-gray-400 mt-auto pt-2 border-t border-dashed border-gray-100">
                  ...
                  <span>
                    <span className="font-medium text-gray-600">
                      {opp.maxApplicants - opp.totalApplications}
                    </span>{" "}
        ...
        <div className="mt-10 text-center">
          <Link
            ...
            className={cn(
              ...
              "gap-2 font-semibold rounded-xl shadow-sm shadow-primary/20"
            )}
          >
