...
export default function OpportunityDetailsContent({
  ...
  const {
    ...
    isError,
    error,
  } = useFetchOpportunityPublicDetails(opportunityId as string);

  if (isError) console.log("Error fetching opportunity details:", error);
  ...
  return (
    <div className="flex flex-col mt-16 gap-5 p-4">
      {/* Header */}
      ...
      <section className="grid grid-cols-2 gap-3">
        <StatCard
          label="Applicants"
          value={selectedOpportunity?.applicants ?? "N/A"}
        />
        <StatCard
          label="Duration"
